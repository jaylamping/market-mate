...
assert any(json.loads(l).get('probe')=='campaign-near-duplicate' and json.loads(l).get('passed') for l in pathlib.Path('.scratch/campaign/probe.log').read_text().splitlines() if l.startswith('{'))
