...
"${compose[@]}" exec -T postgres psql -X -qAt -v ON_ERROR_STOP=1 -U mm -d market_mate < db/fixtures/experiment_dispatch_retry_probe.sql > evidence/incubator-experiment/dispatch-retry.log
...
r['sha256']={p:hashlib.sha256(pathlib.Path(p).read_bytes()).hexdigest() for p in ['db/migrations/0081_campaign_experiment_throughput.sql','db/fixtures/experiment_dispatch_retry_probe.sql','backend/src/incubator_experiment.rs','backend/src/incubator_output.rs']}
