...
export function ModelsTable() {
  ...
  return <div className="grid min-w-0 gap-4">
    <div className="flex flex-wrap items-end justify-between gap-3"><div><div className="flex items-center gap-1"><h3 className="font-medium">Model preferences</h3><Tooltip.Provider delayDuration={200}><Tooltip.Root open={executionHelpOpen} onOpenChange={setExecutionHelpOpen}><Tooltip.Trigger asChild><button type="button" aria-label="Model execution availability" className="grid min-h-11 min-w-11 place-items-center rounded-sm text-muted-foreground hover:text-foreground focus-visible:outline-2 focus-visible:outline-ring" onClick={()=>setExecutionHelpOpen(open=>!open)}><Info className="size-4" aria-hidden="true"/></button></Tooltip.Trigger><Tooltip.Portal><Tooltip.Content side="bottom" sideOffset={6} className="z-50 max-w-xs rounded-md border border-border bg-popover p-3 text-xs leading-relaxed text-popover-foreground shadow-md">OpenRouter execution is available for approved free models. Cursor is catalog-only until its execution adapter is connected. An unavailable first choice blocks the POC; it does not silently switch providers.</Tooltip.Content></Tooltip.Portal></Tooltip.Root></Tooltip.Provider></div><p className="text-sm text-muted-foreground">{policy?.models.length??0} models selected · {counts.openrouter+counts.cursor} provider selections</p></div></div>
    <p className="text-sm text-muted-foreground">Select providers for each model. Move selected providers up or down to set priority; number 1 is first. </p>
    ...
    {save.isError&&<div role="alert" className="text-sm text-destructive"><p>{save.error.message}</p>{conflict&&<><p>Loading the latest preferences discards your unsaved edits.</p><Button variant="link" onClick={()=>void reload()}>Load latest preferences</Button></>}{!conflict&&draft&&<Button variant="link" onClick={()=>persist(draft)}>Retry saving</Button>}</div>}
    ...
    <div className="overflow-hidden rounded-md border"><Table className="min-w-[960px]"><TableHeader><TableRow>{sortHeader("name","Model")}{sortHeader("selected","Providers / Priority")}{sortHeader("date","Release Date","OpenRouter catalog-added date, not a verified original release date.")}{sortHeader("input","Input / 1M","Primary provider base price; OpenRouter reference when missing")}{sortHeader("output","Output / 1M","Primary provider base price; OpenRouter reference when missing")}{sortHeader("context","Context","Primary provider context; OpenRouter reference when missing")}</TableRow></TableHeader><TableBody>{filtered.slice(currentPage*20,currentPage*20+20).map(m=>{
      ...
      return <TableRow key={m.id}><TableCell className="min-w-56 align-top"><div className="pt-3 font-medium"><ModelLink provider={m.metadata.provider} id={m.metadata.id} name={m.name}/></div><span className="mt-1 block text-xs text-muted-foreground">{m.id}</span></TableCell><TableCell className="min-w-72"><div className="grid gap-1">{orderedOffers.map(o=>{
        ...
        const disabled=save.isPending||conflict||!query.data||query.isError;
