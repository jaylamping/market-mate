...
export function parseRuns(v: unknown): Run[] {
  ...
  return array(body.runs).map(v => {
    ...
    const result: Run={ run_key:string(r.run_key), assignment_id:string(r.assignment_id), created_at:date(r.created_at), updated_at:date(r.updated_at), state:state(r.state),
