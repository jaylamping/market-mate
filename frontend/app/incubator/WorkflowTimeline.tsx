...
export function WorkflowTimeline({run,compact=false,evaluation}:{run:Run;compact?:boolean;evaluation?:Evaluation}) {
 ...
 if(run.state==="completed"&&evaluation){
  ...
  return <div className="min-w-0">{!compact&&<p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Workflow</p>}<ProgressFooter compact={compact} steps={[{label:"Assigned",state:"complete",at:run.created_at},{label:"Research",state:"complete",at:run.events.find(e=>e.state==="dispatched")?.at??null},{label:status==="failed"?"Failed":status==="indeterminate"?"Unknown":status==="awaiting_clarification"?"Clarify":status==="needs_input"?"Your input":"Evaluate",state:status==="failed"?"failed":status==="indeterminate"||status==="needs_input"?"paused":status==="evaluating"||status==="awaiting_clarification"?"active":finished?"complete":"pending",at:evaluation.steps[0]?.created_at??null},{label:status==="advance"?"Advanced":status==="refine"?"Refine":status==="close"?"Closed":"Decision",state:finished?"complete":"pending",at:finished?evaluation.steps.at(-1)?.finished_at:null}]}/></div>;
 }
 if(compact){
