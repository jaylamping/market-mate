...
export type CampaignAgenda = { ordinal: number; generation_id: number; creator_model?: string; check?: {issues?:string[];matches?:{id:string;title:string;reason:string;run_key:string}[]}; title: string; premise?: string; state: string; reason: string | null; run_key: string | null;
...
export type Campaign = {
  ...
  creator_usage?:{lifetime:Usage;last_24h:Usage}; creator_calls?:{id:number;model:string;state:string;reason?:string|null;response_text?:string|null;validation_error?:string|null;response_truncated?:boolean;cost_usd:number|null;usage:{prompt_tokens?:number;completion_tokens?:number}|null}[];
...
const stateLabel: Record<string,string> = {pending:"Created",checking:"Checking similarity",queued:"Queued",duplicate:"Duplicate",blocked:"Failed",cancelled:"Cancelled"};
...
function CandidateCard({candidate,model}:{candidate:CampaignAgenda;model:string}) {
  ...
  const explanation = candidate.reason ?? (candidate.state === "checking" ? "Similarity checking is in progress. Research has not started." : "This proposal is waiting for a free research worker. Research begins after similarity checking completes.");
...
export function campaignCandidates(campaign:Campaign|null|undefined, view:string, search="", status="all") {
  return (campaign?.agenda??[]).filter(candidate=>!candidate.run_key && (view==="duplicates" ? candidate.state==="duplicate" : view==="current" && candidate.state!=="duplicate") && (status==="all"||campaignCandidateStatus(candidate.state)===status) && [candidate.title,candidate.premise,candidate.creator_model??campaign?.creator_model,String(candidate.ordinal)].join(" ").toLowerCase().includes(search.toLowerCase()));
}
