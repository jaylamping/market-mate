...
async function response(r: Response): Promise<Campaign> {
  ...
  if (!r.ok) throw Error(body.error ?? "Seed unavailable");
  return body;
}
...
function RetryCandidate({candidate}:{candidate:CampaignAgenda}) {
  ...
  return <div className="mb-5 space-y-2 text-sm">
    {candidate.retry_candidate?<p>Retry queued as proposal #{candidate.retry_candidate}. The original check remains recorded.</p>:<>
      ...
      <p className="text-muted-foreground">{candidate.retry_blocker??"Creates a linked proposal with a new similarity check under the current seed limits. A paused seed stays paused until enabled."}</p>
    </>}
  ...
  return <Dialog.Root>
    <article className="flex min-w-0">
      <Dialog.Trigger asChild>
        <button type="button" className={ticketCardStyles.surface} aria-label={`Open seed ticket ${candidate.title}, ${state}`}>
...
function campaignBacklogEmpty(view:CampaignView):string {
  switch (view) {
    ...
    case "archived": return "No archived seed checks.";
    case "current": return "No current seed backlog.";
    default: {
...
export function CampaignBacklog({campaign,view="current",search="",status="all"}:{campaign:Campaign|null|undefined;view?:CampaignView;search?:string;status?:string}) {
  ...
  const heading = view==="duplicates" ? "Duplicate proposals" : view==="archived" ? "Archived seed checks" : "Seed backlog";
...
export function ResearchCampaign() {
  ...
  const campaignActionLabel = save.isPending ? "Saving…" : stopping ? "Stopping generation…" : !c?.enabled ? "Enable seed" : c.creator_in_progress ? "Stop generation" : "Pause seed";
  return <section className="mb-6 rounded-xl border border-border bg-card p-5" aria-label="Research seed">
    <div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-lg font-semibold">Research seed</h2><span className="text-sm text-muted-foreground">{c ? c.enabled ? "Enabled" : "Paused" : "Loading"}</span></div>
    ...
    {query.isError && <p role="alert" className="mt-3 text-sm text-destructive">Seed could not be refreshed. <button className="underline" onClick={()=>query.refetch()}>Retry</button></p>}
    {c && <>
...
export function CampaignDialog() {
 ...
 return <Dialog.Root><Dialog.Trigger asChild><Button className="min-h-11" variant="outline" aria-label={active ? "Seed generating" : "Seed"}>{active?<LoaderCircle className="animate-spin" aria-hidden="true"/>:<Plus aria-hidden="true"/>}{active?"Seed generating":"Seed"}</Button></Dialog.Trigger>
  <Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-50 bg-black/60"/>
   <Dialog.Content className="fixed left-1/2 top-1/2 z-50 max-h-[calc(100dvh-2rem)] w-[calc(100%-2rem)] max-w-3xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-2xl border border-border bg-background p-6 text-foreground shadow-xl">
    <Dialog.Title className="pr-10 text-xl font-semibold">Seed controls</Dialog.Title>
    ...
    <Dialog.Close asChild><Button variant="ghost" size="icon" className="absolute right-3 top-3 min-h-11 min-w-11" aria-label="Close seed"><X/></Button></Dialog.Close>
    <ResearchCampaign/>
   </Dialog.Content>
  </Dialog.Portal>
 </Dialog.Root>;
