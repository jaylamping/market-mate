...
export function RunCard({run,initiallyOpen=false,initialVersion="current",evaluation,evaluationHistory=[],fallbacks=[]}:{fallbacks?:Run[];run:Run;initiallyOpen?:boolean;initialVersion?:string;evaluation?:Evaluation;evaluationHistory?:Evaluation[]}) {
  ...
  return <Dialog.Root open={open} onOpenChange={setOpen}><div className="relative flex min-w-0 flex-col"><Dialog.Trigger asChild>
    <button type="button" className="group flex min-h-72 w-full min-w-0 flex-1 flex-col items-start rounded-xl border border-border bg-card p-5 text-left text-card-foreground transition-colors hover:border-primary/60 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-ring" aria-label={`Open ${run.config.input.title}, ${stateLabel(run)}, ${time(run.created_at)}`}>
      ...
      <span className="mt-3 line-clamp-2 break-all text-xs text-muted-foreground">{run.config.model}</span>{fallbacks.map(attempt=><span key={attempt.run_key} className="mt-2 text-xs text-muted-foreground">Fallback attempt: {stateLabel(attempt)}</span>)}
...
export function IncubatorPage() {
  ...
  const scopedRuns=researchTickets(runs).filter(run=>Boolean(run.archived)===(archiveView==="archived"));
  ...
  return <div className="supervisory-overview" data-display-only="false" data-order-authority="none"><a className="skip-link" href="#incubator-main">Skip to incubator</a><AppSidebar activePage="/incubator"/>
    <main className="overview-main" id="incubator-main" tabIndex={-1}>
      ...
      {!!query.data?.length && <p className="py-3 text-xs leading-relaxed text-muted-foreground">Showing {visibleRuns.length} of {scopedRuns.length} {archiveView==="archived"?"archived research tickets · Latest 100 archived tickets.":"current assignments · All active work and the latest 100 finished or restored tickets."}</p>}
