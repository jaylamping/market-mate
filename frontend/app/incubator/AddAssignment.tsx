...
export function AddAssignment() {
 ...
 async function submit(checked:RequestCheck,accept=false) {
   ...
   try {
     ...
     setNotice("Assignment queued. Its workflow will update automatically.");
 ...
 async function receive(value:unknown) {
   ...
   if(next.run_key){setNotice("This assignment is already queued.");setOpen(false);revise();return;}
   if(!next.result){setPhase("checking");return;}
   if(needsWarning(next)){setPhase("warning");setNotice("Review the similarity warning in Add assignment before proceeding.");}else await submit(next);
 }
 ...
 return <><Dialog.Root open={open} onOpenChange={setOpen}><Dialog.Trigger asChild><Button className="min-h-11"><Plus aria-hidden="true"/>Add assignment</Button></Dialog.Trigger>
 ...
 <Dialog.Content className="fixed left-1/2 top-1/2 z-50 max-h-[calc(100dvh-2rem)] w-[calc(100%-2rem)] max-w-xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-2xl border border-border bg-background p-6 text-foreground shadow-xl">
  <Dialog.Title className="pr-10 text-xl font-semibold">Add assignment</Dialog.Title>
  <Dialog.Description className="mt-2 text-sm text-muted-foreground">Describe a research question. We’ll check existing assignments, then queue it with your chosen model.</Dialog.Description>
  <Dialog.Close asChild><Button variant="ghost" size="icon" className="absolute right-3 top-3 min-h-11 min-w-11" aria-label="Close new assignment"><X/></Button></Dialog.Close>
  <form className="mt-6 space-y-5" onSubmit={e=>{e.preventDefault();void start();}}>
   ...
   <label className="block space-y-2 text-sm font-medium">Assignment model<select value={model} disabled={locked||routing.isPending} onChange={e=>{setModel(e.target.value);revise();}} className="min-h-11 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="">Use default{routing.data?.default_model?` · ${routing.data.default_model}`:""}</option>{models.map(m=><option key={m.model_id} value={m.model_id}>{m.model_id}</option>)}</select></label>
   {routing.isError&&<p role="alert" className="text-sm text-destructive">Model choices are unavailable. The saved default will be checked when you submit.</p>}
   ...
   {phase==="warning"&&check?.result&&<section aria-label="Similarity warning" className="space-y-3 rounded-lg border border-[var(--warning)]/50 bg-[var(--warning)]/5 p-4">
    ...
    {check.result.matches.map(m=><article key={m.id} className="space-y-1 border-t border-border pt-3 text-sm"><p className="font-medium">{m.title}</p><p className="text-xs text-muted-foreground">{m.state.replaceAll("_"," ")}</p><p>{m.reason}</p>{m.run_key?<a className="text-primary underline underline-offset-4" href={`/incubator?run=${encodeURIComponent(m.run_key)}`} target="_blank" rel="noreferrer">View assignment ↗</a>:<details><summary className="cursor-pointer text-primary">View preserved assignment</summary><p className="mt-2 break-words">{m.text}</p><p className="mt-2 break-all text-xs text-muted-foreground">{m.id}</p></details>}</article>)}
