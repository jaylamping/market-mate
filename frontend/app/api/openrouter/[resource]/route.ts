...
async function forward(request: Request, resource: string, method: "GET" | "PUT") {
  ...
  try {
    const response = await fetch(`${base}/openrouter/${resource}`, {method, cache:"no-store",signal:AbortSignal.timeout(12_000), ...(method === "PUT" ? {headers:{"Content-Type":"application/json"},body:JSON.stringify(await request.json())} : {})});
    return Response.json(await response.json(),{status:response.status,headers:{"Cache-Control":"no-store"}});
  } catch { return Response.json({error:"OpenRouter connector unavailable"},{status:503,headers:{"Cache-Control":"no-store"}}); }
}
