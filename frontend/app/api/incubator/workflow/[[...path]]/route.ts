...
export async function POST(request:Request,{params}:Context){const {path=[]}=await params;if(path.length!==2||!/^\d+$/.test(path[0])||!["answer","dataset","experiment-answer","acquisition","data-request"].includes(path[1]))return new Response(null,{status:404});return requestProxy(request,`/workflow/${path[0]}/${path[1]}`,"POST");}
