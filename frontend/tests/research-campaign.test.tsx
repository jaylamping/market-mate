...
import {CampaignBacklog,campaignCandidates,candidateProgress,ResearchCampaign,CampaignDialog,type CampaignAgenda} from "../app/incubator/ResearchCampaign";
...
test("fresh empty backlog renders the independent Ticket Creator picker",()=>{
 ...
 assert.deepEqual(campaignCandidates(retried,"current").map(c=>c.ordinal),[1,2,4,6]);
 assert.deepEqual(campaignCandidates(retried,"archived").map(c=>c.ordinal),[3]);
