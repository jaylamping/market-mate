...
export function parseCheck(value:unknown):RequestCheck {
 ...
 for(const m of r.matches)if(!m||[m.id,m.title,m.text,m.state,m.reason].some(s=>typeof s!=="string")||(m.run_key!==null&&typeof m.run_key!=="string"))throw Error("Invalid similar assignment");
 return v;
}
...
export async function assignmentRequest(path:string,body?:unknown):Promise<unknown> {
 ...
 const value=await response.json();if(!response.ok)throw Error(value.error??"Assignment request failed");return value;
}
