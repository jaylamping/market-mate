...
export type Agent = {
  ...
  current: {status: "eligible" | "held" | "blocked"; route?: {provider_id: string; model_id: string; tier: Tier; ordinal: number}; held_until?: string; reason?: string; skipped?: {provider_id:string;model_id:string;tier:Tier;reason:string;next_eligible_at?:string}[]};
};
...
export type Revision = {revision_id:number; entity:"provider"|"agent"; entity_id:string; revision:number; source:"import"|"ui"|"api"|"seed"; actor:string; diff:Record<string, unknown>; created_at:string};
