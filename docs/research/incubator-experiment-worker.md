...
Setup can ask the original research model one question, then accept one owner answer. At most three Setup calls, one research clarification and one Experiment call are permitted per ticket. Each call uses existing provider order and zero-spend enforcement. Setup and Experiment use their role preferences, falling back to Default. Original research clarification uses the assignment's explicit model. No raw price observations are sent to models.
