...
| Agent driver | Provider registry, quota windows, agents and tiered routes, and dispatch lineage; `backend/src/driver`, `backend/src/bin/agent-driver.rs`, [agent-driver migration](../db/migrations/0088_agent_driver.sql). OpenRouter spending admission still uses `openrouter_capacity` SQL inside the driver. |
...
5. Dispatch goes through the agent driver, which records each research and later-stage intent, route, attempt, reservation, and outcome. [Request adaptation](../backend/src/openrouter_request.rs) respects model capabilities without removing output or cost bounds.
