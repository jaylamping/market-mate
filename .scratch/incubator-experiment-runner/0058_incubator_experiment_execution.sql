...
CREATE TABLE incubator_experiment_event (
 ...
 state text NOT NULL CHECK(state IN ('awaiting_data','preparing','needs_input','ready','dispatching','running','completed','failed','indeterminate')),
...
DECLARE prior uuid; s research_snapshot%ROWTYPE; BEGIN
 ...
 IF read_incubator_experiment(id_value)->>'status' IS NULL OR read_incubator_experiment(id_value)->>'status' NOT IN ('awaiting_setup','awaiting_data') THEN RAISE EXCEPTION 'experiment_not_waiting_for_data'; END IF;
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path=pg_catalog,public AS $$
 ...
 WHERE e.state IS NULL OR e.state IN ('preparing','ready','dispatching','running') OR (e.state='awaiting_data' AND EXISTS(SELECT 1 FROM incubator_experiment_dataset d WHERE d.experiment_id=t.evaluation_id)) ORDER BY t.evaluation_id LIMIT 1
$$;
CREATE FUNCTION record_incubator_experiment_event(id_value bigint,state_value text,detail_value jsonb) RETURNS void
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public AS $$
DECLARE prior incubator_experiment_event%ROWTYPE; lineage jsonb; registration experiment_preregistration%ROWTYPE; spec jsonb; d jsonb; BEGIN
 PERFORM pg_advisory_xact_lock(58001,id_value::integer);
 ...
 IF FOUND AND prior.state=state_value AND prior.detail=detail_value THEN RETURN; END IF;
 IF NOT coalesce(( (prior.state IS NULL AND state_value IN ('preparing','failed'))
 OR (prior.state='awaiting_data' AND state_value IN ('ready','failed'))
 OR (prior.state='preparing' AND state_value IN ('awaiting_data','ready','needs_input','failed','indeterminate'))
 ...
 IF state_value IN ('preparing','dispatching') AND (detail_value->'request'->>'max_tokens' IS DISTINCT FROM '2048' OR detail_value->'request'->'provider'->'max_price' IS DISTINCT FROM '{"prompt":0,"completion":0}'::jsonb) THEN RAISE EXCEPTION 'bounded_model_request_required'; END IF;
