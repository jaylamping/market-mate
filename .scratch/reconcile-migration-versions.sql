BEGIN; LOCK TABLE schema_migration IN ACCESS EXCLUSIVE MODE; DO $$ BEGIN IF (SELECT count(*) FROM schema_migration WHERE (version=60 AND name='manual_assignment_paid_models' AND checksum='a5ff5d10da4368cce4c71ffe50a77dbd53bd308be00dd500b77701ffb2f5bc6d') OR (version=61 AND name='manual_assignment_model_identifiers' AND checksum='223b410fec5e57e41e76d401cbbca1632192f3d3df9312beee1ccc67ebae7623'))<>2 OR EXISTS(SELECT 1 FROM schema_migration WHERE version>61) THEN RAISE EXCEPTION 'unexpected migration state'; END IF; END $$;
UPDATE schema_migration SET version=62 WHERE version=61; UPDATE schema_migration SET version=61 WHERE version=60;
...
INSERT INTO schema_migration(version,name,checksum,applied_at) VALUES(60,'market_data_storage','d7e4ded24896381c59ce03b4019a7298e9da1251b3fb82904102cb6eff0db312',clock_timestamp());
COMMIT;
