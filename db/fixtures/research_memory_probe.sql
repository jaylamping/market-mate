...
BEGIN
 ...
 -- and repeats collapse and never corroborate.
 ...
 -- and authority semantics rejected.
 PERFORM pg_temp.contract_assert(
 ...
 -- Shared builders for the lesson rows below.
 prov := jsonb_build_object(
 ...
 EXCEPTION WHEN SQLSTATE '22023' THEN
  PERFORM pg_temp.contract_assert(true, 'reject_changed_inputs'); END;
 BEGIN PERFORM propose_research_lesson(
 ...
 EXCEPTION WHEN SQLSTATE '22023' THEN
  PERFORM pg_temp.contract_assert(true, 'reject_gate_change_guidance'); END;
 BEGIN PERFORM propose_research_lesson(
 ...
 EXCEPTION WHEN SQLSTATE '22023' THEN
  PERFORM pg_temp.contract_assert(true, 'reject_own_lineage_support'); END;
END $$;
...
BEGIN
 ...
 -- merely proposed lesson cannot be superseded.
 PERFORM propose_research_lesson(
 ...
 -- Successor linkage is verified: unknown and self successors raise.
...
END $$;
ROLLBACK;
SELECT jsonb_build_object('probe', 'research-memory', 'passed', true, 'checks', jsonb_build_array(
 ...
 'pure_support_rejects_repeated_assignment',
 ...
 'reject_changed_inputs', 'reject_missing_lineage',
 ...
 'pinned_review_flags', 'dissent_versioned_preserved',
