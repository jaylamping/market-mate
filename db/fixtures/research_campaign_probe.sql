...
SELECT pg_temp.assert(read_incubator_campaign()->'agenda'->2->>'state'='cancelled','paused completed check cancelled');
SELECT set_incubator_campaign(true,10,3,3,'vendor/creator:free',10);
RESET ROLE;
UPDATE incubator_campaign SET next_at=now()-interval '1 minute';
SET LOCAL ROLE incubator_runner;
TRUNCATE candidate;
